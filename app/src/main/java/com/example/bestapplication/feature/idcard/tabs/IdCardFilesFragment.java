...
public class IdCardFilesFragment extends Fragment {
    ...
    private volatile String pdfRelPath;
        ...
        AppExecutors.io().execute(() -> {
            try {
                ...
                if (id == null || id.trim().isEmpty()) {
                    item = AppDatabase.get(requireContext()).dao().findFirstItemByType(DocumentTypeIds.IDCARD_CN);
                } else {
                    item = AppDatabase.get(requireContext()).dao().findItem(id);
                }
                if (item == null) {
                    pdfRelPath = null;
                    return;
                ...
                BlobRefEntity pdf = AppDatabase.get(requireContext()).dao().findBlobBySlot(item.itemId, BlobSlots.IDCARD_PDF);
