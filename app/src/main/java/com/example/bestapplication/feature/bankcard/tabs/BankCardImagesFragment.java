...
public class BankCardImagesFragment extends Fragment {
    ...
    private void loadImages() {
        ...
        if (ctx == null || itemId == null || itemId.trim().isEmpty()) return;

        AppExecutors.io().execute(() -> {
            try {
                BlobRefEntity front = AppDatabase.get(ctx).dao().findBlobBySlot(itemId, BlobSlots.BANKCARD_FRONT);
                BlobRefEntity back = AppDatabase.get(ctx).dao().findBlobBySlot(itemId, BlobSlots.BANKCARD_BACK);
