...
public class DocumentTypeAdapter extends RecyclerView.Adapter<DocumentTypeAdapter.VH> {
    ...
    private static String simplifyName(String name) {
        if (name == null) return "";
        // Remove Chinese parentheses: 身份证（大陆） -> 身份证
        return name.replaceAll("（.*?）", "").trim();
    }

    private static int pickColor(String typeId) {
        // UI-only. You can move these into resources later if needed.
        if (DocumentTypeIds.IDCARD_CN.equals(typeId)) return Color.parseColor("#87CEFA");
        if (DocumentTypeIds.BANK_CARD.equals(typeId)) return Color.parseColor("#AFEEEE");
        if (DocumentTypeIds.DRIVER_LICENSE.equals(typeId)) return Color.parseColor("#FFB6C1");
        if (DocumentTypeIds.STUDENT_ID.equals(typeId)) return Color.parseColor("#E6E6FA");
        if (DocumentTypeIds.WORK_ID.equals(typeId)) return Color.parseColor("#C0C0C0");
