        ...
        AppExecutors.io().execute(() -> {
            try {
                ...
                if (itemId == null || itemId.trim().isEmpty()) {
                    item = AppDatabase.get(ctx).dao().findFirstItemByType(DocumentTypeIds.IDCARD_CN);
                } else {
                    item = AppDatabase.get(ctx).dao().findItem(itemId);
                }
                if (item == null) return;

                BlobRefEntity front = AppDatabase.get(ctx).dao().findBlobBySlot(item.itemId, BlobSlots.IDCARD_FRONT);
                BlobRefEntity back = AppDatabase.get(ctx).dao().findBlobBySlot(item.itemId, BlobSlots.IDCARD_BACK);
